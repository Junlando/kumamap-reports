...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .pref-meta-row .dot { color: var(--border); }
    .pref-meta-row strong { color: var(--ink); font-weight: 700; }
    ...
    .stats-col { order: 1; }
    .map-col   { order: 2; width: 100%; }
    .list-col  { order: 3; }

    /* Desktop: 左地図・右stats+list の grid */
    @media (min-width: 900px) {
      ...
      .stats-col { grid-row: 1; grid-column: 2; order: unset; }
      .list-col  { grid-row: 2; grid-column: 2; order: unset; }
    }
    ...
    .list-more-btn:hover { background: #fef2f2; }
...
<body>
  ...
  <div class="page-wrapper">
    ...
    <div class="pref-hero">
      ...
      <div class="pref-meta-row">
        <span>過去30日 <strong id="pref-meta-30days">-</strong>件</span>
        <span class="dot">・</span>
        <span><strong id="pref-meta-views">-</strong> 回閲覧</span>
        <span class="dot">・</span>
        <span>最終更新: <strong id="pref-meta-updated">-</strong></span>
      </div>
    </div>
  ...
  <script>
    ...
    // 日本の座標範囲チェック（異常値を除外）
    function isValidJapanCoord(lat, lng) {
    ...
    function initMap() {
      ...
      // CSS transform アニメーション等で ResizeObserver が発火しないケースの保険
      [100, 300, 600, 1200].forEach(ms => setTimeout(() => leafletMap.invalidateSize(), ms));
    }
    ...
    function renderMapMarkers(items, todayStartSec) {
      ...
      sorted.forEach(item => {
        ...
        if (!isValidJapanCoord(lat, lng)) return;
    ...
    function renderIncidentList(incidents) {
      ...
      const preview = sorted.slice(0, LIST_PREVIEW);
      const hasMore = sorted.length > LIST_PREVIEW;

      el.innerHTML = buildIncidentHtml(preview)
        + (hasMore ? `<button class="list-more-btn" id="list-more-btn">全${sorted.length}件を表示</button>` : "");

      if (hasMore) {
        document.getElementById("list-more-btn").addEventListener("click", () => {
          el.innerHTML = buildIncidentHtml(sorted);
          el.addEventListener("click", incidentClickHandler(sorted));
        });
      }
    ...
    function incidentClickHandler(sorted) {
      return e => {
    ...
    async function loadIncidents() {
      try {
        ...
        const thirtyDaysAgoSec  = targetMidnightSec - 29*DAY_SECONDS;
        ...
        // 30日分をまとめて dedupe。都道府県別サマリーはこれまで通り7日分、
        // このページ自身の見出し（過去30日件数など）は30日分を使う。
        const dedupeMap = new Map();
        [...dataItems,...apiItems]
          .filter(i=>(i.timestamp_second||0)>=thirtyDaysAgoSec)
          .filter(i=>isValidJapanCoord(parseFloat(i.latitude), parseFloat(i.longitude)))
          .forEach(i=>{
        ...
        const allItems30 = Array.from(dedupeMap.values());
        const allItems7  = allItems30.filter(i=>i.timestamp_second>=sevenDaysAgoSec);
        renderPrefSummary(allItems7);

        const pref30d    = allItems30.filter(i=>(i.prefecture||"").toLowerCase()===TARGET_PREF);
        const all7d      = pref30d.filter(i=>i.timestamp_second>=sevenDaysAgoSec);
        ...
        // 出没が少ない県だと今日/3日間/7日間が0で並んで動いていないように見えるので、
        // 7日間が0件のときだけ 1日間/7日間/30日間 という広い窓に切り替える。
        const isLowFrequency = all7d.length === 0;
        document.getElementById("stats-label-1").textContent = isLowFrequency ? "1日間"  : "今日";
        document.getElementById("stats-label-2").textContent = isLowFrequency ? "7日間"  : "3日間";
        document.getElementById("stats-label-3").textContent = isLowFrequency ? "30日間" : "7日間";
        document.getElementById("stats-today").textContent  = todayItems.length;
        document.getElementById("stats-3days").textContent  = isLowFrequency ? all7d.length   : items3d.length;
        document.getElementById("stats-7days").textContent  = isLowFrequency ? pref30d.length : all7d.length;

        // 過去30日件数・最終更新（クロールが動いていることが一目でわかるように）
        document.getElementById("pref-meta-30days").textContent = pref30d.length;
        const latestCreatedMs = pref30d.reduce((max,i)=>{
          if (!i.created_at) return max;
          const t = new Date(i.created_at + "Z").getTime();
          return Number.isFinite(t) && t>max ? t : max;
        }, 0);
        document.getElementById("pref-meta-updated").textContent =
          latestCreatedMs ? formatRelativeTime(latestCreatedMs) : "-";

        // 閲覧数（GA4集計を毎日反映した静的スナップショット。厳密なリアルタイム値ではない）
        if (resViews && resViews.ok) {
          try {
            const viewsData = await resViews.json();
            const v = viewsData[TARGET_PREF];
            if (v != null) document.getElementById("pref-meta-views").textContent = v.toLocaleString();
          } catch (e) { /* pageviews.json が読めなくても致命的ではないので無視 */ }
        }
