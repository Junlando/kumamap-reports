...
function formatDate(ev) {
  ...
  if (ev.endDate) {
    ...
    if (span > 7) return `${y}年${m}月${day}日 〜 ${e.getMonth()+1}月${e.getDate()}日`;
    return `${y}年${m}月${day}日・${e.getDate()}日（${dow}/${DOW_JA[e.getDay()]}）`;
  }
...
function formatDateTag(ev) {
  ...
  if (ev.endDate) {
    const e = new Date(ev.endDate + 'T00:00:00');
    return `${m}月${d.getDate()}日〜${e.getMonth()+1}月${e.getDate()}日`;
  }
...
function buildInfoCard(ev) {
  let rows = '';
  rows += `\n    <div class="info-row"><div class="info-label">開催日</div><div class="info-value">${formatDate(ev)}</div></div>`;
  rows += `\n    <div class="info-row"><div class="info-label">都道府県</div><div class="info-value">${esc(ev.location)}</div></div>`;
  if (ev.time)        rows += `\n    <div class="info-row"><div class="info-label">打上時間</div><div class="info-value">${esc(ev.time)}</div></div>`;
  if (ev.venue)       rows += `\n    <div class="info-row"><div class="info-label">会場</div><div class="info-value">${esc(ev.venue)}</div></div>`;
  if (ev.count)       rows += `\n    <div class="info-row"><div class="info-label">打上発数</div><div class="info-value">${esc(ev.count)}</div></div>`;
  if (ev.paid_seats !== undefined)
    rows += `\n    <div class="info-row"><div class="info-label">有料席</div><div class="info-value">${ev.paid_seats ? 'あり' : 'なし'}</div></div>`;
  if (ev.yatai !== undefined)
    rows += `\n    <div class="info-row"><div class="info-label">屋台</div><div class="info-value">${ev.yatai ? 'あり' : 'なし'}</div></div>`;
  if (ev.access)      rows += `\n    <div class="info-row"><div class="info-label">アクセス</div><div class="info-value">${esc(ev.access)}</div></div>`;
  if (ev.rain)        rows += `\n    <div class="info-row"><div class="info-label">雨天対応</div><div class="info-value">${esc(ev.rain)}</div></div>`;
  if (ev.note)        rows += `\n    <div class="info-row"><div class="info-label">備考</div><div class="info-value">${esc(ev.note)}</div></div>`;
  return `  <div class="info-card">\n    <div class="info-card-title">開催情報</div>${rows}\n  </div>`;
}
...
function buildMapSection(ev) {
  ...
  return `  <div class="map-card">
    <iframe src="https://maps.google.com/maps?q=${ev.lat},${ev.lng}&z=15&output=embed&hl=ja" width="100%" height="300" style="border:0;border-radius:12px;display:block" allowfullscreen loading="lazy"></iframe>
    <a class="map-link" href="${openUrl}" target="_blank" rel="noopener">🗺️ Googleマップで開く</a>
  </div>\n`;
}

const outDir = path.join(BASE_DIR, 'spot/ja');
fs.mkdirSync(outDir, { recursive: true });

let count = 0;
for (const ev of EVENTS) {
  const canonical   = `${SITE_ROOT}/spot/ja/${ev.id}.html`;
  const zhCanonical = `${SITE_ROOT}/spot/${ev.id}.html`;
  const prefUrl     = `${SITE_ROOT}/pref/ja/${ev.ken}.html`;
  const isLong      = ev.endDate && (new Date(ev.endDate+'T00:00:00') - new Date(ev.date+'T00:00:00')) / 86400000 > 7;

  const title   = `${ev.name} | ${ev.location}花火大会2026 | Junlando`;
  const metaDesc = `${ev.note ? ev.note + ' ' : ''}${formatDate(ev)}。${ev.venue ? '会場：' + ev.venue + '。' : ''}アクセス・開催情報を詳しく紹介。`;
...
<head>
  ...
  <meta property="og:title" content="${esc(ev.name)} | 花火大会2026" />
...
<div class="event-header">
  <div class="event-header-inner">
    ...
    <div class="event-tags">
      ...
      ${isLong ? '<span class="tag-long">⭐ 長期開催</span>' : ''}
...
${buildMapSection(ev)}
  <a class="back-btn" href="../../ja/">← 花火大会カレンダーに戻る</a>
  <p class="updated-date">最終更新：${BUILD_DATE}</p>
</div>
</body>
</html>`;
...
console.log(`✅ JA spot: ${count} pages → spot/ja/`);
