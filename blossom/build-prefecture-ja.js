...
function timelineCard(fc, flower) {
  ...
  const isKoyo = flower === 'koyo';
  const badgeMap = {
    upcoming: { bg:'#e8f4fd', color:'#1976d2', txt: isKoyo ? '紅葉前' : '開花前' },
    blooming: { bg:'#e8f5e9', color:'#2e7d32', txt: isKoyo ? '色づき中 🍁' : '開花中 💠' },
    peak:     { bg:'#fff3e0', color:'#ef6c00', txt: isKoyo ? '見頃！ 🔥' : '満開！ 🔥' },
    ended:    { bg:'#f5f5f5', color:'#757575', txt: 'シーズン終了' },
  ...
  return \`
    <div class="timeline-card">
      <div class="card-header">
        <h2>\${fc.name || prefNameEn} \${FLOWERS[${JSON.stringify(flower)}].label} \${FLOWER_YEAR[${JSON.stringify(flower)}]}年 開花予報</h2>
      </div>
      <div class="timeline-grid">
        <div class="timeline-item">
          <div class="timeline-label">\${isKoyo ? '色づき開始' : '開花日'}</div>
          <div class="timeline-date">\${fc.bloom ? fmtShort(fc.bloom, ${JSON.stringify(flower)}) : '—'}</div>
        </div>
        <div class="timeline-item">
          <div class="timeline-label">\${isKoyo ? '見頃ピーク' : '満開日'}</div>
        ...
        <div class="timeline-item">
          <div class="timeline-label">\${isKoyo ? '紅葉終了' : 'シーズン終了'}</div>
          <div class="timeline-date">\${fc.end ? fmtShort(fc.end, ${JSON.stringify(flower)}) : '—'}</div>
        </div>
      </div>
      \${fc.bloom ? \`
      <div class="progress-wrap">
        <div class="progress-label"><span>シーズン進捗</span><span>\${pct}%</span></div>
        <div class="progress-bar"><div class="progress-fill" style="width:\${pct}%"></div></div>
      </div>\` : ''}
      \${fc.forecastRange ? \`
      <div style="padding:10px 20px;border-top:1px solid var(--border)">
        <div style="font-size:12px;color:var(--text-sub)">📊 予測期間 <strong style="color:var(--text)">\${fc.forecastRange}</strong></div>
      </div>\` : ''}
    </div>\`;
}
  ...
  const html = spots.map(s => {
    ...
    <a class="spot-card" href="\${spotUrl(flower, s.name)}">
      <div class="spot-info">
        ...
        \${period ? \`<div class="spot-period-row">見頃時期：<span>\${period}</span></div>\` : ''}
      </div>
      <div class="spot-arrow">›</div>
    </a>\`;
  }).join('');
  return \`<div class="spots-title">おすすめスポット</div>\${html}\`;
}
...
function nearbySection(forecastPrefs, currentId, flower) {
  ...
  const STATUS_LABEL = { upcoming:'開花前', blooming:'開花中', peak:'満開！', ended:'終了', none:'—' };
  const cards = others.map(([id, fc]) => {
    const s = getStatus(fc.bloom, fc.peak, fc.end);
    const jaName = fc.name || id;
    return \`
      <a class="nearby-card" href="../\${flower}/\${id}.html">
        <div class="nearby-name">\${jaName}</div>
  ...
  return \`<div class="nearby-title">他の都道府県</div><div class="nearby-grid">\${cards}</div>\`;
}
...
async function render() {
  ...
  try {
    const [forecast, spotsData, detailEnData] = await Promise.all([
      ...
      fetch(\`../../../data/spots/detail-\${flower}.ja.json\`).then(r => r.json()).catch(() => ({})),
      getSpotIdMap(),
    ]);
    ...
    if (!fc) {
      ...
      app.innerHTML = \`<div class="error-msg"><h2>No data found for "\${ken}"</h2>
        <p style="margin-top:8px"><a href="../../../ja/index.html" style="color:var(--active)">← 開花予報に戻る</a></p></div>\`;
      return;
    }
