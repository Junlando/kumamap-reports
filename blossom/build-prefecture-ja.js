...
function statusBanner(fc, flower) {
  ...
  const msgs = {
    upcoming: () => { const d = daysUntil(fc.bloom); return d > 0 ? \`開花予定日：<strong>\${fmtShort(fc.bloom, flower)}</strong>（あと\${d}日）\` : isKoyo ? 'もうすぐ色づき始めます' : 'もうすぐ開花します'; },
    blooming: () => \`現在\${isKoyo ? '色づき中' : '開花中'}。満開予定：<strong>\${fmtShort(fc.peak, flower)}</strong>\`,
    peak:     () => \`\${isKoyo ? '見頃' : '満開'}です！見頃は<strong>\${fmtShort(fc.end, flower)}</strong>まで\`,
    ended:    () => \`\${fmtShort(fc.end, flower)}にシーズン終了\`,
    none:     () => 'この都道府県の予報データはありません',
  };
