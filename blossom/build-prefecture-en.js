/**
 * build-prefecture-en.js
 * 生成英文縣市頁面：prefecture/en/ajisai/hokkaido.html 等
 * 執行方式：node build-prefecture-en.js
 */
...
const FLOWERS = {
  ajisai: { label: 'Hydrangea',      labelFull: 'Japan Hydrangea',      year: 2026, emoji: '💠', color: '#7c5cbf', light: '#f0eafb' },
  koyo:   { label: 'Autumn Foliage', labelFull: 'Japan Autumn Foliage', year: 2026, emoji: '🍁', color: '#d4570a', light: '#fff0e6' },
  sakura: { label: 'Cherry Blossom', labelFull: 'Japan Cherry Blossom', year: 2027, emoji: '🌸', color: '#d46b8a', light: '#fff0f5' },
};
...
function loadPrefDescEn(flower) {
  const p = path.join(BASE_DIR, `data/prefecture/detail-${flower}.en.json`);
  return fs.existsSync(p) ? JSON.parse(fs.readFileSync(p, 'utf8')) : {};
}
...
function genHtml({ ken, flower, flowerInfo, fc, detailEn, prefDescEn }) {
  const prefNameEn  = PREF_EN[ken] || ken;
  const prefNameJp  = fc.name || ken;
  const range       = fc.forecastRange || '';
  const prefDesc    = prefDescEn[ken] || {};
  const canonical   = `${SITE_ROOT}/prefecture/en/${flower}/${ken}.html`;
  const zhCanonical = `${SITE_ROOT}/prefecture/${flower}/${ken}.html`;
  const homeUrl     = `${SITE_ROOT}/en/index.html?flower=${flower}`;
  ...
  const desc = prefDesc.tagline
    ...
    : `Best ${flowerInfo.label.toLowerCase()} spots in ${prefNameEn}, Japan ${flowerInfo.year}.${descRange} Bloom forecast, recommended spots & access info.`;
  ...
  const jsonldBreadcrumb = JSON.stringify({
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": prefNameEn, "item": canonical },
    ]
  });
...
<head>
  ...
  <meta property="og:title" content="${esc(prefNameEn)} ${flowerInfo.label} ${flowerInfo.year} | Japan" />
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      ...
      --gray: #f5f6f8; --border: #e8e9ec; --text: #1a1a2e; --text-sub: #6b7280; --radius: 12px;
    }
    ...
    .progress-fill { height:100%; border-radius:99px; background:var(--active); }
...
<header>
  <div class="header-inner">
    ...
    <a class="back-btn" href="../../../en/index.html?flower=${flower}">← Flower Forecast</a>
  </div>
</header>

<div class="breadcrumb-bar">
  <div class="breadcrumb-inner">
    <a href="../../../en/index.html?flower=${flower}" style="color:var(--text-sub);text-decoration:none">Flower Forecast</a>
    <span style="margin:0 6px">›</span>
    <span>${esc(prefNameEn)}</span>
  </div>
</div>
...
<footer>
  <p>© 2026 Junlando | <a href="../../../en/">Home</a> · <a href="../../../en/index.html">Flower Forecast</a> · <a href="https://community.junlando.com" target="_blank" rel="noopener">Travel Forum</a></p>
...
const PREF_EN = ${JSON.stringify(PREF_EN)};
...
function spotUrl(flower, name) {
  ...
  return id ? \`../../../spot/en/\${flower}/\${id}.html\` : \`../../../spot.html?flower=\${flower}&spot=\${encodeURIComponent(name)}\`;
}

const FLOWERS = {
  ajisai: { label: 'Hydrangea',      emoji: '💠', color: '#7c5cbf', light: '#f0eafb' },
  koyo:   { label: 'Autumn Foliage', emoji: '🍁', color: '#d4570a', light: '#fff0e6' },
  sakura: { label: 'Cherry Blossom', emoji: '🌸', color: '#d46b8a', light: '#fff0f5' },
};
...
function statusBanner(fc, flower) {
  const s = getStatus(fc.bloom, fc.peak, fc.end);
  const f = FLOWERS[flower];
  const isKoyo = flower === 'koyo';
  const msgs = {
    upcoming: () => { const d = daysUntil(fc.bloom); return d > 0 ? \`Expected to start <strong>\${fmtShort(fc.bloom, flower)}</strong> — \${d} days away\` : isKoyo ? 'Color change starting soon' : 'Bloom starting soon'; },
    blooming: () => \`Currently \${isKoyo ? 'turning colors' : 'blooming'} — peak expected <strong>\${fmtShort(fc.peak, flower)}</strong>\`,
    peak:     () => \`Peak \${isKoyo ? 'color' : 'bloom'} now! Best viewing until <strong>\${fmtShort(fc.end, flower)}</strong>\`,
    ended:    () => \`Season ended \${fmtShort(fc.end, flower)}\`,
    none:     () => 'No forecast data available for this prefecture',
  };
  const icons = { upcoming:'📅', blooming: f.emoji, peak:'🔥', ended:'🍃', none:'ℹ️' };
  return \`
    <div class="status-banner \${s}">
      <span class="icon">\${icons[s]}</span>
      <span class="msg">\${(msgs[s] || msgs.none)()}</span>
    </div>\`;
}
...
function timelineCard(fc, flower) {
  ...
  const isKoyo = flower === 'koyo';
  const badgeMap = {
    upcoming: { bg:'#e8f4fd', color:'#1976d2', txt: 'Coming Soon' },
    blooming: { bg:'#e8f5e9', color:'#2e7d32', txt: isKoyo ? 'Turning 🍁' : 'Blooming 💠' },
    peak:     { bg:'#fff3e0', color:'#ef6c00', txt: 'Peak 🔥' },
    ended:    { bg:'#f5f5f5', color:'#757575', txt:'Ended' },
    none:     { bg:'#f5f5f5', color:'#aaa',    txt:'—' },
  }[s];
  const prefNameEn = PREF_EN[${JSON.stringify(ken)}] || ${JSON.stringify(ken)};
  return \`
    <div class="timeline-card">
      <div class="card-header">
        <h2>\${prefNameEn} \${FLOWERS[${JSON.stringify(flower)}].label} \${FLOWER_YEAR[${JSON.stringify(flower)}]} Schedule</h2>
      </div>
      <div class="timeline-grid">
        <div class="timeline-item">
          <div class="timeline-label">\${isKoyo ? 'Color Start' : 'First Bloom'}</div>
          <div class="timeline-date">\${fc.bloom ? fmtShort(fc.bloom, ${JSON.stringify(flower)}) : '—'}</div>
        </div>
        <div class="timeline-item">
          <div class="timeline-label">\${isKoyo ? 'Peak Color' : 'Peak Bloom'}</div>
          <div class="timeline-date">\${fc.peak ? fmtShort(fc.peak, ${JSON.stringify(flower)}) : '—'}</div>
          <div class="timeline-badge" style="background:\${badgeMap.bg};color:\${badgeMap.color}">\${badgeMap.txt}</div>
        </div>
        <div class="timeline-item">
          <div class="timeline-label">Season End</div>
          <div class="timeline-date">\${fc.end ? fmtShort(fc.end, ${JSON.stringify(flower)}) : '—'}</div>
        </div>
      </div>
      \${fc.bloom ? \`
      <div class="progress-wrap">
        <div class="progress-label"><span>Season Progress</span><span>\${pct}%</span></div>
        <div class="progress-bar"><div class="progress-fill" style="width:\${pct}%"></div></div>
      </div>\` : ''}
      \${fc.forecastRange ? \`
      <div style="padding:10px 20px;border-top:1px solid var(--border)">
        <div style="font-size:12px;color:var(--text-sub)">📊 Forecast range <strong style="color:var(--text)">\${fc.forecastRange}</strong></div>
      </div>\` : ''}
    </div>\`;
}

function spotsSection(spots, detailEn, flower) {
  if (!spots || spots.length === 0) return '';
  const html = spots.map(s => {
    const d = detailEn[s.name] || {};
    const period = d.period || s.period || '';
    const romaji = d.romaji || '';
    const displayName = romaji ? \`\${romaji} <span style="font-size:12px;color:#6b7280;font-weight:400">(\${s.name})</span>\` : s.name;
    return \`
    <a class="spot-card" href="\${spotUrl(flower, s.name)}">
      <div class="spot-info">
        <div class="spot-name">\${displayName}</div>
        \${s.address ? \`<div class="spot-address">📍 \${s.address}</div>\` : ''}
        \${period ? \`<div class="spot-period-row">Best season: <span>\${period}</span></div>\` : ''}
      </div>
      <div class="spot-arrow">›</div>
    </a>\`;
  }).join('');
  return \`<div class="spots-title">Recommended Spots</div>\${html}\`;
}

function nearbySection(forecastPrefs, currentId, flower) {
  const others = Object.entries(forecastPrefs).filter(([id]) => id !== currentId).slice(0, 8);
  if (!others.length) return '';
  const STATUS_LABEL = { upcoming:'Coming Soon', blooming:'Blooming', peak:'Peak', ended:'Ended', none:'—' };
  const cards = others.map(([id, fc]) => {
    const s = getStatus(fc.bloom, fc.peak, fc.end);
    const enName = PREF_EN[id] || id;
    return \`
      <a class="nearby-card" href="../\${flower}/\${id}.html">
        <div class="nearby-name">\${enName}</div>
        <div class="nearby-date">\${fc.bloom ? fmtShort(fc.bloom, flower) : '—'}</div>
        <div style="font-size:11px;margin-top:3px;color:var(--text-sub)">\${STATUS_LABEL[s]}</div>
      </a>\`;
  }).join('');
  return \`<div class="nearby-title">Other Prefectures</div><div class="nearby-grid">\${cards}</div>\`;
}
...
async function render() {
  ...
  try {
    const [forecast, spotsData, detailEnData] = await Promise.all([
      fetch(\`../../../data/forecast/\${flower}-\${year}.json\`).then(r => r.json()),
      fetch(\`../../../data/spots/\${flower}.json\`).then(r => r.json()).catch(() => ({})),
      fetch(\`../../../data/spots/detail-\${flower}.en.json\`).then(r => r.json()).catch(() => ({})),
      getSpotIdMap(),
    ]);
    ...
    if (!fc) {
      ...
      app.innerHTML = \`<div class="error-msg"><h2>No data found for "\${ken}"</h2>
        <p style="margin-top:8px"><a href="../../../en/index.html" style="color:var(--active)">← Back to Flower Forecast</a></p></div>\`;
      return;
    }

    app.classList.remove('loading');
    app.innerHTML = \`
      \${statusBanner(fc, flower)}
      \${timelineCard(fc, flower)}
      \${spotsSection(spotsData[ken] || [], detailEnData, flower)}
      \${nearbySection(forecast.prefectures, ken, flower)}
    \`;
  } catch (e) {
    console.error(e);
    app.classList.remove('loading');
    app.innerHTML = \`<div class="error-msg"><h2>Failed to load. Please try again.</h2></div>\`;
  }
}
...
for (const [flower, flowerInfo] of Object.entries(FLOWERS)) {
  ...
  const prefDescEn  = loadPrefDescEn(flower);
  const outDir      = path.join(BASE_DIR, 'prefecture', 'en', flower);
  fs.mkdirSync(outDir, { recursive: true });

  let count = 0;
  for (const [ken, fc] of Object.entries(forecast)) {
    const html = genHtml({ ken, flower, flowerInfo, fc, detailEn, prefDescEn });
    fs.writeFileSync(path.join(outDir, `${ken}.html`), html, 'utf8');
    allPages.push({ url: `${SITE_ROOT}/prefecture/en/${flower}/${ken}.html` });
    count++;
  }
  console.log(`✅ en/${flower}: ${count} prefectures → prefecture/en/${flower}/`);
}

// 寫 sitemap
const sitemapPath = path.join(BASE_DIR, 'sitemap-en.xml');
let sitemap = fs.existsSync(sitemapPath) ? fs.readFileSync(sitemapPath, 'utf8') : `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n</urlset>`;
const newEntries = allPages.map(({ url }) => `  <url>\n    <loc>${url}</loc>\n    <lastmod>${BUILD_DATE}</lastmod>\n    <changefreq>weekly</changefreq>\n    <priority>0.65</priority>\n  </url>`).join('\n');
sitemap = sitemap.replace('</urlset>', `${newEntries}\n</urlset>`);
fs.writeFileSync(sitemapPath, sitemap, 'utf8');

console.log(`\n📦 Total: ${allPages.length} English prefecture pages`);
