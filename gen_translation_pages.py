#!/usr/bin/env python3
"""生成翻譯落地頁靜態 HTML（SEO 優化版）"""

import os

# slug, from_code, to_code, h1, title_tag, meta_description, html_lang
PAIRS = [
    (
        "vi-to-zh-tw",
        "Vietnamese", "Chinese (Traditional)",
        "越南語翻譯中文",
        "越南語翻譯中文 App | 越南文翻中文免費工具",
        "越南語翻譯成中文，越南文翻中文即時翻譯，支援照片翻譯、語音輸入。免費越南語翻譯 App，比 Google 翻譯更自然。",
        "zh-Hant",
    ),
    (
        "zh-tw-to-vi",
        "Chinese (Traditional)", "Vietnamese",
        "越南文翻譯・中文翻越南語",
        "越南文翻譯 App | 免費中文翻越南語工具",
        "免費中文翻越南文翻譯 App，支援越南語即時翻譯、照片翻譯、語音朗讀。翻譯越南語比 Google 更準確，越南語言翻譯首選。",
        "zh-Hant",
    ),
    (
        "ja-to-zh-tw",
        "Japanese", "Chinese (Traditional)",
        "日文翻中文・日文照片翻譯",
        "日文翻中文 App | 日文照片翻譯・菜單翻譯免費工具",
        "免費日文翻中文翻譯 App，支援日文照片翻譯、菜單翻譯。即時翻譯、語音輸入，日語翻譯成繁體中文，快速準確。",
        "zh-Hant",
    ),
    (
        "zh-tw-to-ja",
        "Chinese (Traditional)", "Japanese",
        "中文翻日文",
        "中文翻日文 App | 免費線上日語翻譯",
        "免費中文翻日文翻譯 App，繁體中文翻譯成日語，支援照片翻譯、語音輸入與朗讀。",
        "zh-Hant",
    ),
    (
        "en-to-zh-tw",
        "English", "Chinese (Traditional)",
        "英文翻中文",
        "英文翻中文 App | 英文翻譯中文繁體免費工具",
        "免費英文翻中文翻譯 App，英文翻譯中文繁體即時完成，支援照片翻譯、語音輸入。英文翻中文線上工具，快速準確。",
        "zh-Hant",
    ),
    (
        "zh-tw-to-en",
        "Chinese (Traditional)", "English",
        "中文翻英文",
        "中文翻英文 App | 免費中翻英線上翻譯",
        "免費中文翻英文 App，中翻英即時翻譯。支援照片翻譯，中翻英句子、中翻英文章均支援，語音朗讀功能。",
        "zh-Hant",
    ),
    (
        "ko-to-zh-tw",
        "Korean", "Chinese (Traditional)",
        "韓文翻中文",
        "韓文翻中文 App | 免費韓語翻譯工具",
        "免費韓文翻中文翻譯 App，韓語翻譯成繁體中文，支援照片翻譯、即時翻譯、語音輸入。",
        "zh-Hant",
    ),
    (
        "zh-tw-to-ko",
        "Chinese (Traditional)", "Korean",
        "中文翻韓文",
        "中文翻韓文 App | 免費韓語翻譯工具",
        "免費中文翻韓文翻譯 App，繁體中文翻譯成韓語，支援照片翻譯、語音輸入與朗讀。",
        "zh-Hant",
    ),
    (
        "ja-to-en",
        "Japanese", "English",
        "日文翻英文",
        "日文翻英文 App | 免費日語英語翻譯",
        "免費日文翻英文翻譯 App，即時將日語翻譯成英語，支援照片翻譯、語音輸入。",
        "zh-Hant",
    ),
    (
        "en-to-ja",
        "English", "Japanese",
        "英文翻日文",
        "英文翻日文 App | 免費英語日語翻譯",
        "免費英文翻日文翻譯 App，即時將英語翻譯成日語，支援照片翻譯、語音輸入與朗讀。",
        "zh-Hant",
    ),
    (
        "vi-to-en",
        "Vietnamese", "English",
        "Dịch Việt Anh",
        "Dịch Việt Anh App | Dịch tiếng Việt sang tiếng Anh chuẩn miễn phí",
        "Dịch tiếng Việt sang tiếng Anh chuẩn, miễn phí. App dịch ảnh, dịch Việt Anh và Anh Việt nhanh chóng. Hỗ trợ giọng nói.",
        "vi",
    ),
    (
        "en-to-vi",
        "English", "Vietnamese",
        "Dịch Anh Việt",
        "Dịch Anh Việt App | Dịch tiếng Anh sang tiếng Việt miễn phí",
        "Dịch tiếng Anh sang tiếng Việt miễn phí. App dịch ảnh, dịch Anh Việt và Việt Anh nhanh chóng, hỗ trợ giọng nói.",
        "vi",
    ),
    (
        "zh-tw-to-th",
        "Chinese (Traditional)", "Thai",
        "中文翻泰文・泰文翻譯中文",
        "中文翻泰文 App | 泰文翻譯中文・照片翻譯免費工具",
        "免費中文翻泰文翻譯 App，泰文翻譯中文即時完成。支援照片翻譯、語音朗讀功能。",
        "zh-Hant",
    ),
    (
        "th-to-zh-tw",
        "Thai", "Chinese (Traditional)",
        "泰文翻中文・中文翻譯泰文",
        "泰文翻中文 App | 泰文翻譯中文・照片翻譯免費工具",
        "免費泰文翻中文翻譯 App，泰文翻譯中文快速準確。支援照片翻譯、語音輸入。",
        "zh-Hant",
    ),
    # 日本市場：タイ語日本語翻訳
    (
        "ja-to-th",
        "Japanese", "Thai",
        "タイ語日本語翻訳・写真翻訳アプリ",
        "タイ語日本語翻訳アプリ | 写真翻訳・日本語からタイ語無料",
        "タイ語日本語翻訳アプリ。写真翻訳でメニュー・看板もOK。日本語からタイ語・タイ語から日本語の両方向に対応。音声入力対応。",
        "ja",
    ),
    # 越南市場：越泰翻譯
    (
        "vi-to-th",
        "Vietnamese", "Thai",
        "Dịch Thái Việt",
        "Dịch Thái Việt App | Dịch tiếng Việt sang tiếng Thái miễn phí",
        "App dịch tiếng Việt sang tiếng Thái và dịch Thái sang Việt miễn phí. Dịch ảnh, dịch Thái Việt nhanh chóng, hỗ trợ giọng nói.",
        "vi",
    ),
    (
        "zh-tw-to-id",
        "Chinese (Traditional)", "Indonesian",
        "中文翻印尼文",
        "中文翻印尼文 App | 免費印尼語翻譯工具",
        "免費中文翻印尼文翻譯 App，繁體中文翻譯成印尼語，支援照片翻譯、即時翻譯。",
        "zh-Hant",
    ),
    (
        "id-to-zh-tw",
        "Indonesian", "Chinese (Traditional)",
        "印尼文翻中文",
        "印尼文翻中文 App | 免費印尼語翻譯工具",
        "免費印尼文翻中文翻譯 App，印尼語翻譯成繁體中文，支援照片翻譯、即時翻譯。",
        "zh-Hant",
    ),
    # 日本市場：ハングル翻訳・韓国語日本語翻訳
    (
        "ko-to-ja",
        "Korean", "Japanese",
        "ハングル翻訳・韓国語日本語翻訳",
        "ハングル翻訳アプリ | 韓国語日本語翻訳・写真翻訳無料",
        "ハングル翻訳アプリ。写真翻訳でメニュー・看板もOK。韓国語→日本語・日本語→韓国語の両方向に対応。無料・即時・音声入力対応。",
        "ja",
    ),
    (
        "ja-to-ko",
        "Japanese", "Korean",
        "히라가나·한자 번역기・일본어 한국어 번역",
        "히라가나·한자 번역기 앱 | 일본어 한국어 번역 무료",
        "히라가나·가타카나·한자를 한국어로 무료 번역. 사진 번역 앱으로 메뉴·간판도 OK. 일본어 한국어 번역 양방향 지원.",
        "ko",
    ),
    # 韓國市場：韓越翻譯
    (
        "ko-to-vi",
        "Korean", "Vietnamese",
        "베트남어 번역기・한국어 베트남어 번역",
        "베트남어 번역기 앱 | 한국어 베트남어 무료 번역",
        "베트남어 번역기 앱으로 한국어↔베트남어 양방향 무료 번역. 사진 번역, 음성 입력 가능.",
        "ko",
    ),
    # 日本市場：越南語日本語翻訳
    (
        "ja-to-vi",
        "Japanese", "Vietnamese",
        "ベトナム語日本語翻訳・写真翻訳",
        "ベトナム語日本語翻訳アプリ | 写真翻訳・ベトナム語翻訳無料",
        "ベトナム語日本語翻訳アプリ。写真翻訳でメニュー・看板もOK。ベトナム語→日本語・日本語→ベトナム語の両方向に対応。音声入力対応。",
        "ja",
    ),
    # 越南市場：越日翻譯
    (
        "vi-to-ja",
        "Vietnamese", "Japanese",
        "Dịch Nhật Việt",
        "Dịch Nhật Việt App | Dịch tiếng Nhật sang tiếng Việt miễn phí",
        "App dịch Nhật Việt và Việt Nhật miễn phí. Dịch ảnh menu, biển hiệu tiếng Nhật ngay tức thì. Hỗ trợ nhập liệu giọng nói.",
        "vi",
    ),
    # 越南市場：越韓翻譯
    (
        "vi-to-ko",
        "Vietnamese", "Korean",
        "Dịch sang tiếng Hàn",
        "Dịch Tiếng Hàn App | Dịch sang tiếng Hàn Quốc miễn phí",
        "App dịch tiếng Hàn miễn phí. Dịch ảnh, dịch sang tiếng Hàn hoặc dịch tiếng Hàn sang tiếng Việt nhanh chóng. Hỗ trợ giọng nói.",
        "vi",
    ),
]

APP_BANNER = {
    "ja": {
        "cta": "写真・メニューも翻訳できる！",
        "sub": "カメラで撮るだけ。日本のメニュー・看板・書類を即翻訳。",
        "btn": "アプリをダウンロード",
        "appstore": "https://apps.apple.com/jp/app/id6742044545",
        "googleplay": "https://play.google.com/store/apps/details?id=com.junlando.japanmenu",
    },
    "ko": {
        "cta": "사진·메뉴도 번역 가능!",
        "sub": "카메라로 찍으면 바로 번역. 일본 메뉴·간판·서류 OK.",
        "btn": "앱 다운로드",
        "appstore": "https://apps.apple.com/kr/app/id6742044545",
        "googleplay": "https://play.google.com/store/apps/details?id=com.junlando.japanmenu",
    },
    "vi": {
        "cta": "Dịch được cả ảnh & menu!",
        "sub": "Chụp ảnh menu, biển hiệu Nhật Bản là dịch ngay tức thì.",
        "btn": "Tải app ngay",
        "appstore": "https://apps.apple.com/app/id6742044545",
        "googleplay": "https://play.google.com/store/apps/details?id=com.junlando.japanmenu",
    },
    "zh-Hant": {
        "cta": "照片、菜單也能翻譯！",
        "sub": "拍照即可翻譯日本菜單、招牌、文件，出遊必備。",
        "btn": "下載 App",
        "appstore": "https://apps.apple.com/tw/app/id6742044545",
        "googleplay": "https://play.google.com/store/apps/details?id=com.junlando.japanmenu",
    },
}

ALL_PAIR_LINKS = "".join(
    f'<a href="/translation/{slug}/" '
    f'class="text-sm text-blue-600 hover:underline bg-white rounded-lg px-3 py-2 border border-gray-100 shadow-sm hover:shadow transition">'
    f'{h1}</a>\n      '
    for slug, _, _, h1, _, _, _ in PAIRS
)

TEMPLATE = """\
<!DOCTYPE html>
<html lang="{html_lang}">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>{title_tag}</title>
  <meta name="description" content="{meta_description}" />
  <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="min-h-screen bg-gradient-to-br from-blue-50 to-white">
  <div class="max-w-4xl mx-auto px-4 py-10">
    <a href="/translation/" class="text-sm text-blue-500 hover:underline mb-6 inline-block">← 所有語言</a>
    <h1 class="text-3xl font-bold text-center text-gray-800 mb-2">{h1}</h1>
    <p class="text-center text-gray-500 text-sm mb-8">{meta_description}</p>

    <div id="translator" data-from="{from_code}" data-to="{to_code}"></div>

    <section class="mt-12 bg-gradient-to-r from-indigo-500 to-blue-500 rounded-2xl p-6 text-white flex flex-col sm:flex-row items-center gap-4">
      <div class="flex-1 text-center sm:text-left">
        <p class="text-xl font-bold mb-1">{banner_cta}</p>
        <p class="text-sm opacity-90">{banner_sub}</p>
      </div>
      <div class="flex flex-col sm:flex-row gap-2 shrink-0">
        <a href="{banner_appstore}" target="_blank" rel="noopener"
           class="flex items-center gap-2 bg-black text-white rounded-xl px-4 py-2 text-sm font-medium hover:bg-gray-900 transition">
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>
          App Store
        </a>
        <a href="{banner_googleplay}" target="_blank" rel="noopener"
           class="flex items-center gap-2 bg-white text-gray-800 rounded-xl px-4 py-2 text-sm font-medium hover:bg-gray-100 transition">
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M3.18 23.76c.3.17.65.19.97.07l11.46-6.62-2.5-2.5-9.93 9zm16.6-9.57l-2.44-1.41-2.77 2.77 2.77 2.77 2.47-1.43c.7-.41.7-1.29-.03-1.7zM2.52.29C2.2.1 1.83.1 1.54.3L13.1 11.87 2.52.29zm1.64 1L13.87 12l-2.5 2.5L3.44 6.57 4.16 1.29z"/></svg>
          Google Play
        </a>
      </div>
    </section>

    <section class="mt-10">
      <h2 class="text-lg font-semibold text-gray-700 mb-4">其他熱門翻譯</h2>
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
      {pair_links}
      </div>
    </section>
  </div>

  <script type="module" src="/translation/translator.js"></script>
</body>
</html>
"""

base = os.path.join(os.path.dirname(__file__), "translation")

for slug, from_code, to_code, h1, title_tag, meta_description, html_lang in PAIRS:
    folder = os.path.join(base, slug)
    os.makedirs(folder, exist_ok=True)
    banner = APP_BANNER[html_lang]
    html = TEMPLATE.format(
        h1=h1,
        title_tag=title_tag,
        meta_description=meta_description,
        from_code=from_code,
        to_code=to_code,
        pair_links=ALL_PAIR_LINKS,
        html_lang=html_lang,
        banner_cta=banner["cta"],
        banner_sub=banner["sub"],
        banner_appstore=banner["appstore"],
        banner_googleplay=banner["googleplay"],
    )
    with open(os.path.join(folder, "index.html"), "w", encoding="utf-8") as f:
        f.write(html)
    print(f"  ✓ {slug}/index.html")

print(f"\n完成！共生成 {len(PAIRS)} 個頁面。")
